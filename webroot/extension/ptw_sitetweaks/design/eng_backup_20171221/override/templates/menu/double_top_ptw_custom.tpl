...
{* PTW - check for custom title for menu item - ref: https://doc.ez.no/eZ-Publish/Technical-manual/4.x/Reference/Template-operators/Data-and-information-extraction/ezini *}
{def $titleOverrideArray=ezini( 'customMenuTitles', 'NodeMenuTitle', 'ptw_sitetweaks_custom_menu_titles.ini' )}

{* ... check only ... disabled *}
{* foreach $titleOverrideArray as $arrayIndex => $titleOverride}
	{$arrayIndex} : {$titleOverride} <br />
{/foreach *}
        ...
        <div class="nav-collapse">
            <ul class="nav">
                {foreach $level_2_items as $key => $item}
                    ...
                    {* PTW Set title, if it does not exist then use (default) item title *}
                    {def $displayTitle=$titleOverrideArray[$item.node_id]|wash()}
                    {* Debug Only:  The Current NODE: {$item.node_id} - string: {$titleOverrideArray[$item.node_id]} <br/> *}
                    {if eq($displayTitle,"")} {set $displayTitle=$item.name|wash()} {/if}
                    {* NOTE ALSO $displayTitle added twice to the lines below *}   
                    
                    {if eq( $item.class_identifier, 'link')}
                        <li id="node_id_{$item.node_id}"{if $item_class} class="{$item_class|implode(" ")}"{/if}><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $item.node_id)|ezurl}{else}href={$item.data_map.location.content|ezurl}{if and( is_set( $item.data_map.open_in_new_window ), $item.data_map.open_in_new_window.data_int )} target="_blank"{/if}{/if}{if $pagedata.is_edit} onclick="return false;"{/if} title="{$item.data_map.location.data_text|wash}" class="menu-item-link" rel={$item.url_alias|ezurl}>{if $item.data_map.location.data_text}{$item.data_map.location.data_text|wash()}{else}{$displayTitle}{/if}</a></li>
                    {else}
                        <li id="node_id_{$item.node_id}"{if $item_class} class="{$item_class|implode(" ")}"{/if}><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $item.node_id)|ezurl}{else}{$item.url_alias|ezurl}{/if}{if $pagedata.is_edit} onclick="return false;"{/if}>{$displayTitle}</a></li>
                    {/if}
                {/foreach}
            </ul>
        </div>
